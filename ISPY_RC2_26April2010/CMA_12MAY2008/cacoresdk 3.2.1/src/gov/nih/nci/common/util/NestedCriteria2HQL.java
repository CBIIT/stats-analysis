...
//					}
	...
	private HashMap getObjAttrCriterion(Object obj, Configuration cfg) throws Exception
	{
		...
		try{
			idField = pclass.getMappedClass().getDeclaredField(identifier);
		}
		catch(NoSuchFieldException e){
			Class objClass = obj.getClass();
			idField = checkForImplicitInheritance(objClass,identifier);
		}
	  ...
	  /**
	   * Add by caIntegrator team - Himanso Sahni 
	   * checkForImplicitInheritance method recursively walks thru 
	   * the object class mappings instead of Hibernate mappings to map
	   * the identifier to the super class.
	   * This allows the NestedCriteria2HQL class to allows users 
	   * to use Hibernate's Implicit Inheritance Strategy Pattern
	   * 
	   */
	private Field checkForImplicitInheritance(Class objClass, String identifier){
		Field idField = null;
		if(objClass != null){
		...
		try{
			if(objClass != null){
				idField = objClass.getDeclaredField(identifier);
			}
		}
		catch(NoSuchFieldException e){
			idField = checkForImplicitInheritance(objClass,identifier);
